...
/**
 * <p>
 * desc:视频会议Activity TODO 需要修改画面为本地摄像头画面
 * 
 * @data 2013-9-1
 * @time 下午10:19:25
 */

public class CallingVideoConfActivity extends BaseActivity implements
NavigationListener, OnCheckedChangeListener{

    private RelativeLayout layout_call_center;
    private ImageView img_head; // 视频会议头像
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_calling_videoconf);
        ...
        if (inCall) {
            ...
            HBaseApp.post2WorkRunnable(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        jsonObj.put(RtcConst.kgvctype, RtcConst.grouptype_multigrpchatAV); //yes
    ...
    private void onResponse_grpcreate(String params) {
        ...
        try {
            if (params == null || params.equals("")) {
                CommFunc.DisplayToast(CallingVideoConfActivity.this, "创建现场直播失败参数为空");
                CommFunc.PrintLog(5, LOGTAG,
            ...
            CommFunc.PrintLog(5, LOGTAG, "OnRestgrpvOptCreate code:" + code
                    + " reason:" + reason);
            if (code.equals("0")) {
                CommFunc.DisplayToast(CallingVideoConfActivity.this, "视频会议创建成功");
                initCallingView();
                initCallingData();

                joinResult = TGroupRecordInfo.CALL_TYPE_OUT_SUCCESS;
            } else {
                CommFunc.PrintLog(5, LOGTAG, "视频会议创建失败:" + code + " reason:"
                        + reason);
                CommFunc.DisplayToast(CallingVideoConfActivity.this, "视频会议创建失败:" + code
                        + " reason:" + reason);
    ...
    private void onResponse_grpgetMemberList(String parameters) {
        ...
        try {
            ...
            if (code.equals("0")) {
                ...
                CommFunc.DisplayToast(CallingVideoConfActivity.this, "获取成员列表失败:"+code+" reason:"+reason);
            }
        } catch (JSONException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        ...
        try {
            ...
            if(jsonrsp.isNull("code")==false)
            {
                ...
                if(code.equals("0"))
                {
                    ...
                    CommFunc.DisplayToast(CallingVideoConfActivity.this, "邀请成员参与群组会话成功:"+code+" reason:"+reason);
                }
                else
                {
                    CommFunc.PrintLog(5, LOGTAG, "邀请成员参与群组会话失败:"+code+" reason:"+reason);
                    CommFunc.DisplayToast(CallingVideoConfActivity.this, "邀请成员参与群组会话失败:"+code+" reason:"+reason);
                }
            }
        ...
        try {
            ...
            if(jsonrsp.isNull("code")==false)
            {
                ...
                if(code.equals("0") || code.equals("200"))
                {
                    ...
                    CommFunc.DisplayToast(CallingVideoConfActivity.this, "踢出成员成功");
                    if(list.size()>0)
                        list.remove(kickposition);
                }
                else
                {
                    CommFunc.DisplayToast(CallingVideoConfActivity.this, "踢出成员失败:"+code+" reason:"+reason);
    ...
    private void handleCallMessage(Message msg) {
        switch (msg.what) {
            ...
            case MsgKey.SLN_CallFailed:
                ...
                CommFunc.DisplayToast(CallingVideoConfActivity.this, "呼叫失败:[" + msg.arg1
                        + "]");
                // closeUI();
                break;
    ...
    private void playRing(int resId, boolean bSpeaker, boolean hanguped) {
        ...
        if ((mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_SILENT)) {
            ...
            mediaPlayer = MediaPlayer.create(CallingVideoConfActivity.this, resId);
            if (mediaPlayer == null) {
                return;
            }
            if (mAudioManager != null) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        Log.e(LOGTAG, "Current audio mode : " + mode);
        ...
        switch (keyCode) {
                case KeyEvent.KEYCODE_VOLUME_UP:// 音量增大
                        ...
                        Log.e(LOGTAG, "Current audio volume up");
                break;
                case KeyEvent.KEYCODE_VOLUME_DOWN:// 音量减小
                        mAudioManager.setStreamVolume(mode, currentVolume-1, 1);
                        Log.e(LOGTAG, "Current audio volume down");
                break;
