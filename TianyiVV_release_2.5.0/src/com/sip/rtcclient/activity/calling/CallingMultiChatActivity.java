 ...
 * ：多人会话群聊，默认类型，此类型下发起方即作为多人的管理方，并且其他成员均为可发言状态；
 */

@SuppressLint("Wakelock") public class CallingMultiChatActivity extends BaseActivity implements NavigationListener{

    private RelativeLayout layout_call_center;
    private ImageView img_head; //多人群聊头像
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_calling_multichat);
        ...
        if (inCall) {
            ...
            HBaseApp.post2WorkRunnable(new Runnable() {
                ...
                public void run() 
                {
                    ...
                    try {
                        jsonObj.put(RtcConst.kgvctype, RtcConst.grouptype_multigrpchatA); //yes
...
//                }
    ...
    private void onResponse_grpcreate(String params)
    {
        try {
            ...
            {
                CommFunc.DisplayToast(CallingMultiChatActivity.this, "创建多人群聊失败参数为空");
            ...
            if(code.equals("0"))
            {
                CommFunc.DisplayToast(CallingMultiChatActivity.this, "多人群聊创建成功");
                initCallingView();
                initCallingData();
                joinResult = TGroupRecordInfo.CALL_TYPE_OUT_SUCCESS;
            }
            else{
                CommFunc.PrintLog(5,LOGTAG,"多人群聊创建失败:"+code+" reason:"+reason);
                CommFunc.DisplayToast(CallingMultiChatActivity.this, "多人群聊创建失败:"+code+" reason:"+reason);
                joinResult = TGroupRecordInfo.CALL_TYPE_OUT_SUCCESS;
                saveGroupRecordInfo();
    ...
    private void onResponse_grpgetMemberList(String parameters)
    {
        ...
        try {
            ...
            else
            {
                ...
                CommFunc.DisplayToast(CallingMultiChatActivity.this, "获取成员列表失败:"+code+" reason:"+reason);
            }
        } catch (JSONException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        ...
        try {
            ...
            if(jsonrsp.isNull("code")==false)
            {
                ...
                if(code.equals("0"))
                {
                    ...
                    CommFunc.DisplayToast(CallingMultiChatActivity.this, "邀请成员参与群组会话成功:"+code+" reason:"+reason);
                }
                else
                {
                    CommFunc.PrintLog(5, LOGTAG, "邀请成员参与群组会话失败:"+code+" reason:"+reason);
                    CommFunc.DisplayToast(CallingMultiChatActivity.this, "邀请成员参与群组会话失败:"+code+" reason:"+reason);
                }
            }
        ...
        try {
            ...
            if(jsonrsp.isNull("code")==false)
            {
                ...
                if(code.equals("0") || code.equals("200"))
                {
                    ...
                    CommFunc.DisplayToast(CallingMultiChatActivity.this, "踢出成员成功");
                    if(list.size()>0)
                    list.remove(kickposition);
                }
                else
                {
                    CommFunc.DisplayToast(CallingMultiChatActivity.this, "踢出成员失败:"+code+" reason:"+reason);
    ...
    private void onResponse_grpMicManagement(String parameters,int type)
    {
        ...
        try {
            ...
            if(jsonrsp.isNull("code")==false)
            {
                ...
                if(code.equals("0"))
                {
                    CommFunc.DisplayToast(CallingMultiChatActivity.this,"mic操作成功");
                }
                else
                {
                    CommFunc.PrintLog(5, LOGTAG, "mic操作失败:"+RtcConst.getStringID(type)+"code:"+code+"reason:"+reason);
                    CommFunc.DisplayToast(CallingMultiChatActivity.this,"mic操作失败:"+RtcConst.getStringID(type)+"code:"+code+"reason:"+reason);
                }
            }
        } catch (JSONException e) {
            // TODO Auto-generated catch block
    ...
    private void handleCallMessage(Message msg) {
        switch (msg.what) {
            ...
            case MsgKey.SLN_CallFailed:
                ...
                CommFunc.DisplayToast(CallingMultiChatActivity.this, "呼叫失败:["+msg.arg1 +"]");
                //closeUI();
                break;
    ...
    private void playRing(int resId, boolean bSpeaker, boolean hanguped) {
        ...
        if ((mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_SILENT)) {
            ...
            mediaPlayer = MediaPlayer.create(CallingMultiChatActivity.this, resId);
            if (mediaPlayer == null) {
                return;
            }
            if (mAudioManager != null) {
    ...
    public void onOpt_InvitedMebmer(String  remoteuri) //邀请成员加入
    {
        ...
        confmgr.groupCall(RtcConst.groupcall_opt_invitedmemberlist,remoteuri); //多人列表取被叫 逗号 间隔
    }
    ...
    public void OnOpt_CloseConf() //关闭会话
    {
        ...
        confmgr.groupCall(RtcConst.groupcall_opt_close, null); //多人列表取被叫 逗号 间隔 
    }
    ...
    public void saveGroupRecordInfo() {
        ...
        info.setConftype(TGroupRecordInfo.TYPE_NMULTICHAT);
