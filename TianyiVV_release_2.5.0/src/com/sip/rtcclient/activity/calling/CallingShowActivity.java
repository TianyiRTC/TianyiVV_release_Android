...
import com.sip.rtcclient.adapter.PopupGridAdapter;
 ...
 * @data 2013-9-1
 * @time 下午10:19:25
 * 3.3.7    多人两方语音创建给普通成员麦克（给麦）
 * 3.3.5    多人两方语音创建者收麦
 * 3.3.6    多人两方语音普通成员自己释麦
 * 
 */

public class CallingShowActivity extends BaseActivity implements NavigationListener, OnClickListener, OnItemClickListener{

    private RelativeLayout layout_call_center;
    private ImageView img_head;	//VV秀场头像
    ...
    private LinearLayout layout_add_speaker;
    ...
    private PopupGridAdapter popupAdapter;
    ...
    private List<TContactInfo> speakerList;
    ...
    private int speaker = 1; // 标识有发言权的成员数量，1标识默认创建者可以发言
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_calling_show);
    ...
    private void initView() {
        ...
        layout_add_speaker = (LinearLayout)findViewById(R.id.group_creator_add_speaker);
        ...
        popupLayout = (LinearLayout) findViewById(R.id.layout_popup);
        popupGridView = (GridView) findViewById(R.id.popup_gridview);
        popupGridView.setSelector(new ColorDrawable(Color.TRANSPARENT)); // 清除黄色背景色
    ...
    private void initData() {
        ...
        speakerList = new ArrayList<TContactInfo>();
        if (groupInfo!= null) {
        ...
        if (inCall) {
            ...
            HBaseApp.post2WorkRunnable(new Runnable() {
                ...
                public void run() 
                {
                    ...
                    try {
                        jsonObj.put(RtcConst.kgvctype, RtcConst.grouptype_multitwoA); //yes
    ...
    private void initCallingView() {
        ...
        if (inCall) {
            ...
            layout_add_speaker.setVisibility(View.GONE);
        }else {
            layout_exit.setVisibility(View.GONE);
            layout_hangup.setVisibility(View.VISIBLE);
            img_members_add.setVisibility(View.VISIBLE);
            layout_add_speaker.setVisibility(View.VISIBLE);
        }
    }

    /**
     * 设置通话中显示的数据
     * TODO 如果底层无群组信息传递过来，需要将if(groupInfo!=null){}代码段注释掉,if(groupInfo!=null)语句块中的代码针对被叫者
     */
    private void initCallingData(){
        if(groupInfo!=null)
        {
           ...
           //     tv_group_members_count.setText("(" + count + ")");   
            ...
            // TODO 设置创建者头像、创建者名称
            TContactInfo contactInfo = SQLiteManager.getInstance().getContactInfoByNumber(groupInfo.getGroupCreator());
            if (contactInfo != null) {
//                if (contactInfo.getPhotoId() != 0) {
//                    Uri uri = ContentUris.withAppendedId(ContactsContract.Contacts.CONTENT_URI, contactInfo.getPhotoId());
//                    InputStream input = ContactsContract.Contacts .openContactPhotoInputStream(getContentResolver(), uri);
//                    Bitmap contactPhoto = BitmapFactory.decodeStream(input);
//                    img_group_creator.setBackgroundDrawable(new BitmapDrawable(contactPhoto));
//                }
        ...
        chronometer.start();
        img_speaker.setImageDrawable(getResources().getDrawable(R.drawable.bg_speaker_inner));
        if (inCall) {
    ...
    public void onExit(View view){
        ...
        MyApplication.getInstance().onConfCallHangup();
        layout_exit.setBackgroundColor(getResources().getColor(R.color.lightgray));
    ...
    private void ParseNotifyInfo(String infos)
    {
        ...
        try {
            ...
            //  CommFunc.PrintLog(5, LOGTAG,"parseNotifyInfo callID:"+callid);
            for (int i = 0; i < mebmberArray.length(); i++) {
                ...
                CommFunc.PrintLog(5, LOGTAG, "ParseNotifyInfo mebmberArray userid:"+userid);
                ...
                if(itemObject.has("memberStatus"))
                {
                    ...
                    //如果剔除的成员当前还在增加发言权中需要刷新发言状态同时将回收发言置为增加发言
                    if(status == 3 || status == 4)
                    {
                        updateSpeakerStatus(0,userid); 
                    }
                ...
                if(itemObject.has("upAudioState"))
                {
                    ...
                    CommFunc.PrintLog(5,LOGTAG,"parseNotifyInfo audioState="+audioState+"userid="+userid);
                    String sStatus = (audioState==0)?"关闭":"开启";
                    CommFunc.DisplayToast(this, "用户:["+userid+"]"+"麦克:"+sStatus);
                    if(inCall && userid!=null && userid.equals(MyApplication.getInstance().getUserID()))
                        updateMicStatus(audioState);
                    if(inCall==false) //对于创建者来说需要维护发言权的数量
                    {
                        updateSpeakerStatus(audioState,userid);
                    }
                }
            }
        } catch (JSONException e) {
    ...
    { 
        CommFunc.PrintLog(1, LOGTAG, "BroadMsg_OnResponse actionn:"+RtcConst.getStringID(action)+"params:"+params);
        switch(action)
        {   
            //rest 相关;
            ...
            default: 
            {
                CommFunc.PrintLog(5, LOGTAG, "BroadMsg_OnResponse unknownmsg"+params);
                break;
            }
        } 
    }
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(CallingActivity.BROADCAST_CALLING_ACTION)) {
                ...
                if (bundle != null) {
                    ...
                    int msgtype = bundle.getInt("arg2",MsgKey.grpv_listener_onResponse); //
    ...
    /**
     * TODO
     * memberArray是解析服务端传输过来的数据进行群组成员的填充，对于创建者来说群组成员直接读取本地的群组数据即可
     * @param params
     */
    private void onResponse_grpcreate(String params)
    {
        ...
        try {
            ...
            {  
                CommFunc.DisplayToast(CallingShowActivity.this, "创建vv秀场失败参数为空");
                CommFunc.PrintLog(5, LOGTAG, "OnRestgrpvOptCreate resopnse strResponse:"+params);
                return;
            }
            JSONObject jsonrsp = new JSONObject(params);
            CommFunc.PrintLog(5, LOGTAG, "OnRestgrpvOptCreate resopnse:"+jsonrsp.toString());
            String code = jsonrsp.getString("code");
            String reason = jsonrsp.getString("reason");
            CommFunc.PrintLog(5, LOGTAG, "OnRestgrpvOptCreate code:"+code+" reason:"+reason);
            if(code.equals("0"))
            {
                CommFunc.DisplayToast(CallingShowActivity.this, "vv秀场创建成功");
                initCallingView();
                initCallingData();
                joinResult = TGroupRecordInfo.CALL_TYPE_OUT_SUCCESS;
            }
            else{
                CommFunc.PrintLog(5,LOGTAG,"vv秀场创建失败:"+code+" reason:"+reason);
                CommFunc.DisplayToast(CallingShowActivity.this, "vv秀场创建失败:"+code+" reason:"+reason);
                joinResult = TGroupRecordInfo.CALL_TYPE_OUT_SUCCESS;
                saveGroupRecordInfo();
                finish();
            }
    ...
    //维护发言权数量 创建者维护 
    private void updateSpeakerStatus(int audiostate,String userid)
    {
        //        CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus audiostate:"+audiostate +"userid:"+userid);
        TContactInfo info = SQLiteManager.getInstance().getContactInfoByNumber(userid);
        //       CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus getContactInfoByNumber:"+info.getPhoneNum());
        if(audiostate == 0)
        {
            //           CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus:remove speaker--before"+speaker+"speakerlist:"+speakerList.size());
            boolean bret = containsObject(speakerList, info);
            if (bret) {               
                //speakerList.remove(info);
                speakerList.clear(); //speakerlist保留一个发言
                //               CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus:remove ret:"+bret+"info:"+info.getPhoneNum()+"speakerlist:"+speakerList.size());
                //  if(bret)
                {  
                    speaker = speaker-1;
                }
                //此时需要刷新创建者图片可以增加发言人 button更换 
                // 图片用的是一张，提示的文字更换
                tv_speaker_add.setText(getString(R.string.vv_show_call_add_speaker));
                //               CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus: audiostate:"+audiostate+" userid:"+userid +" speaker"+speaker+"speakerlist:"+speakerList.size());

            }
        }
        else
        {
            if (containsObject(speakerList, info)==false) {

                boolean bret = speakerList.add(info);
                //               CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus:add ret:"+bret+"info:"+info.getPhoneNum());
                if(bret)
                    speaker = speaker+1;
                //                CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus: audiostate:"+audiostate+" userid:"+userid +" speaker"+speaker+"speakerList:"+speakerList);
                //                CommFunc.PrintLog(5, LOGTAG, "updateSpeakerStatus: add speaker"+speaker+"speakerlist:"+speakerList.size());
            }
        }
    }
    //更新 mic开启 关闭状态
    ...
    private void onResponse_grpgetMemberList(String parameters)
    {
        ...
        try {
            ...
            else
            {
                ...
                CommFunc.DisplayToast(CallingShowActivity.this, "获取成员列表失败:"+code+" reason:"+reason);
            }
        } catch (JSONException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
    ...
    {
        CommFunc.PrintLog(5, LOGTAG, "onResponse_grpvInvitedMemberList resopnse:"+parameters);
        ...
        try {
            ...
            if(jsonrsp.isNull("code")==false)
            {
                ...
                if(code.equals("0"))
                {
                    ...
                    CommFunc.DisplayToast(CallingShowActivity.this, "邀请成员参与群组会话成功:"+code+" reason:"+reason);                   
                }
                else
                {
                    CommFunc.PrintLog(5, LOGTAG, "邀请成员参与群组会话失败:"+code+" reason:"+reason);
                    CommFunc.DisplayToast(CallingShowActivity.this, "邀请成员参与群组会话失败:"+code+" reason:"+reason);
                }
            }
    ...
    {
        CommFunc.PrintLog(5, LOGTAG, "onResponse_grpvkickedMemberList resopnse :"+parameters);
        ...
        try {
            ...
            if(jsonrsp.isNull("code")==false)
            {
                ...
                if(code.equals("0") || code.equals("200"))
                {
                    ...
                    CommFunc.DisplayToast(CallingShowActivity.this, "踢出成员成功");
                    if(list.size()>0)
                    list.remove(kickposition);
                }
                else
                {
                    CommFunc.DisplayToast(CallingShowActivity.this, "踢出成员失败:"+code+" reason:"+reason);
    ...
    {  
        CommFunc.PrintLog(5, LOGTAG, "onResponse_grpvMicManagement resopnse :"+parameters);

        CommFunc.PrintLog(5, LOGTAG, "onResponse_grpvMicManagement:"+RtcConst.getStringID(type));
        try {
            if(parameters == null || parameters.equals(""))
                return;
            JSONObject jsonrsp = new JSONObject(parameters);
            if(jsonrsp.isNull("code")==false)
            {
                ...
                //  Utils.PrintLog(5, LOGTAG, "onResponse_grpvMicManagement code:"+code+" reason:"+reason);
                if(code.equals("0"))
                {
                    CommFunc.PrintLog(5, LOGTAG, "mic操作成功");
                    CommFunc.DisplayToast(CallingShowActivity.this,"mic操作成功");
                }
                else
                {
                    CommFunc.PrintLog(5, LOGTAG, "mic操作失败:"+RtcConst.getStringID(type)+"code:"+code+"reason:"+reason);
                    CommFunc.DisplayToast(CallingShowActivity.this,"mic操作失败:"+RtcConst.getStringID(type)+"code:"+code+"reason:"+reason);
                }
            }
        } catch (JSONException e) {
            // TODO Auto-generated catch block
    ...
    {
        CommFunc.PrintLog(5, LOGTAG, "onResponse_grpvCloseGrpv resopnse strResponse:"+parameters);
    ...
    private void handleCallMessage(Message msg) {
        switch (msg.what) {
            ...
            case MsgKey.SLN_CallFailed:
                ...
                CommFunc.DisplayToast(CallingShowActivity.this, "呼叫失败:["+msg.arg1 +"]");
                //closeUI();
                break;
    ...
    private void playRing(int resId, boolean bSpeaker, boolean hanguped) {
        ...
        if ((mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_SILENT)) {
            ...
            mediaPlayer = MediaPlayer.create(CallingShowActivity.this, resId);
            if (mediaPlayer == null) {
                return;
            }
            if (mAudioManager != null) {
    ...
    /**
     * TODO 增加发言人与回收发言权的最终结果
     */
    @Override
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if (view.getId() == R.id.layout_popup_gridview) {
            if(list.size() <= position);
            else if (tv_speaker_add.getText().toString().equals(getString(R.string.vv_show_call_add_speaker))) {
                TContactInfo info = list.get(position);
                tv_speaker_add.setText(getString(R.string.vv_show_call_release_speaker));
               // String uri = info.getName().equals(null)?info.getPhoneNum():info.getName();
                String uri = info.getPhoneNum();
                OnOpt_MicManaget(1, uri);
                CommFunc.PrintLog(5,LOGTAG, "增加[" + uri+"]为发言者 speakerList:"+speakerList.size()+"position:"+position+"uri:"+uri);
                //    			speakerList.add(info); //此处放在通知刷新处理
                //                ++ speaker;
            }else {
                 TContactInfo info = speakerList.get(position);


                tv_speaker_add.setText(getString(R.string.vv_show_call_add_speaker));
               // String uri = info.getName().equals(null)?info.getPhoneNum():info.getName();
                String uri = info.getPhoneNum();
                OnOpt_MicManaget(0, uri);
                CommFunc.PrintLog(5,LOGTAG, "已收回[" + uri + "]的发言权");
                CommFunc.PrintLog(5, LOGTAG, "回收发言 speakerList:"+speakerList.size()+"position:"+position+"uri:"+uri);
     ...
     * 增加发言人window
     */
    private void showSpeakerAddWindow() {
        CommFunc.PrintLog(5, LOGTAG, "showSpeakerAddWindow:speaker"+speaker+"speakerList:"+speakerList.size());

        if (!isPopup) {
            if (speaker ==1) {
                if(list.size() == 0)
                {
                    CommFunc.DisplayToast(this, "无其他用户可选");
                    return;
                }
                popupAdapter = new PopupGridAdapter(this, list);
            }
            else if(speaker==2) {
                //回收发言权ui
                ...
                popupAdapter = new PopupGridAdapter(this, speakerList);
            }
            popupGridView.setAdapter(popupAdapter);
    ...
    public void onAddSpeaker(View view){
        if (!inCall) {	// 判断是否为创建者点击，只有创建者才有权增加发言成员
            showSpeakerAddWindow();
        }
    }
    ...
    private void toSpeak() {
        ...
        tv_transfer.setText(getString(R.string.vv_show_call_speak_tip));
        img_speaker.setBackgroundResource(R.drawable.bg_speaker_circle_blue);
        // canSpeak = true; 在实际刷新后进行赋值
        //do something
    }

    /**
     * 被赋予发言权的成员放弃麦克(放弃发言权)刷新界面
     */
    ...
    private void showGiveUpDialog() {
        ...
        dialog.setMessageText(getString(R.string.ok),getString(R.string.cancel),getString(R.string.vv_show_call_speak_give_up));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.calling_show_speaker:	//如果是被创建者赋予发言权的成员点击，弹出放弃发言权对话框
                if (canSpeak && inCall) {
                    showGiveUpDialog();
                }
                break;
    ...
    public void onOpt_KickMember(String  remoteuri) //踢出成员
    {
        ...
        GroupMgr confmgr = MyApplication.getInstance().getGrpMgr();
        if(confmgr==null)
            return;

        confmgr.groupCall(RtcConst.groupcall_opt_kickedmemberlist,remoteuri); //多人列表取被叫 逗号 间隔 
    }
    ...
    {  
        CommFunc.PrintLog(5, LOGTAG, "OnOpt_MicManaget(0 mute ):"+opttype +" remote:"+remoteuri);
        GroupMgr grpmgr = MyApplication.getInstance().getGrpMgr();
        if(grpmgr==null)
            return;        
    ...
    public void saveGroupRecordInfo() {
        ...
        info.setCallId(String.valueOf(CommFunc.dateToLang(startDate + " " + startTime)));
        info.setStartDate(startDate);
        info.setEndDate(CommFunc.getEndDate());
        info.setStartTime(startTime);
        info.setEndTime(CommFunc.getEndTime());
        //info.setTime(String.valueOf(CommFunc.getTotalTime(startTime, info.getEndTime())));
    ...
    public  boolean containsObject(List<TContactInfo> list, TContactInfo info) {
        CommFunc.PrintLog(5, "showActivity", "containsObject: listsize"+list.size()+"info:"+info.getPhoneNum());
        for (int i = 0; i < list.size(); i++) {

            CommFunc.PrintLog(5, "showActivity", "list phone"+list.get(i).getPhoneNum());
            if (list.get(i).getPhoneNum().equals(info.getPhoneNum())) {
                CommFunc.PrintLog(5, "showActivity", "containsObject return true");
                return true;
            }
        }
