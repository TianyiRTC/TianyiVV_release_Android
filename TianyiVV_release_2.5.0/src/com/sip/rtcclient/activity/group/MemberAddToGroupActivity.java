 ...
 * 添加到分组Activity
 */
public class MemberAddToGroupActivity extends BaseActivity implements OnTianyiTitleActed, OnCheckedChangeListener, Observer {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_member_add_to_group);
	...
	private void initView() {
		...
		layout_bottom = (RelativeLayout) findViewById(R.id.activity_group_more_bottom);
	...
	private void initData() {
		...
		titleView.setTitle(-1, getString(R.string._group_add_member_to_group));
		cb_select_all.setOnCheckedChangeListener(this);
		list = SQLiteManager.getInstance().getAllGroupInfoAExept(number);
	...
	public void onAdd(View view){
		CommFunc.PrintLog(5, LOGTAG, "onAddMemberToGroup");
		if (selectSet.size() > 0) {
			SQLiteManager.getInstance().addMemberToGroups(selectSet, number);
		}
		selectSet.clear();
		updateView();
	}
